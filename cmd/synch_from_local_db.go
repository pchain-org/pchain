...
func localSync(srcChain, toChain *core.BlockChain, stop *chan struct{}) error {

	done := false
	startWith := toChain.CurrentBlock().NumberU64()
	startWith++
	...
	for !done {
		...
		err := importBlockResult(toChain, blocks)
		if err != nil {
			return err
		}
...
func importBlockResult(toChain *core.BlockChain, blocks types.Blocks) error {

	if index, err := toChain.InsertChain(blocks); err != nil {
...
func sfldInitEnv(ctx *cli.Context) error {
	...
	types.SynchFromLocalDB = true

	sldVars.chainId = ctx.GlobalString(ChainIdFlag.Name)
