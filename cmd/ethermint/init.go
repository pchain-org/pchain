...
func initEthGenesis(ctx *cli.Context) error {
	...
	var coreGenesis = core.Genesis{
		...
		Coinbase: common.ToHex((*validators[0]).EtherumePubKey.Address()),
	...
	for i,validator := range validators {
		...
		otherEthPub, r := validator.EtherumePubKey.(crypto.EtherumPubKey)
		if l&&r {
			fmt.Println("this is create")
			coreGenesis.Alloc[common.ToHex(validator.EtherumePubKey.Address())] = struct {
				Code    string
				Storage map[string]string
				Balance string
				Nonce   string
			...
			fmt.Println(reflect.TypeOf(validator.EtherumePubKey))
		}
...
func createGenesisDoc(coreGenesis *core.Genesis) error {
	...
	if _, err := os.Stat(genFile); os.IsNotExist(err) {
		...
		_, _, checkErr := checkAccount(*coreGenesis)
		if(checkErr != nil) {
		...
		for k,v := range coreGenesis.Alloc {
			...
			genDoc.CurrentEpoch.Validators[idx] = types.GenesisValidator{EthAccount: common.HexToAddress(k), PubKey:crypto.BLSPubKey(common.FromHex(v.ConsensusPubKey)), Amount:amount}
			idx ++
		}
...
func createPriValidators(num int) []*types.PrivValidator {
	...
	for i:=0; i < num; i++ {
		...
		privKey := validators[i].EtherumePrivKey.(crypto.EtherumPrivKey)
...
func checkAccount(coreGenesis core.Genesis) (common.Address, int64, error) {
	fmt.Println("coinbase:",coreGenesis.Coinbase)
