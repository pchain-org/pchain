...
//----------------------------------------------------------------------
/**/
func (b *Backend) AccumulateRewards(rewardPerBlock *big.Int) {
	b.pending.accumulateRewards(rewardPerBlock)
}

func (p *pending) accumulateRewards(rewardPerBlock *big.Int) {
	p.commitMutex.Lock()
	defer p.commitMutex.Unlock()
	// set the epoch reward per block
	p.work.rewardPerBlock = rewardPerBlock
	p.work.accumulateRewards()
}

func (w *work) accumulateRewards() {
/*
