...
const MaxVotesAggrSize = 22020096 // 21MB TODO make it configurable

type VoteAggr struct {
...
func newVotesAggr(height int64, round int, type byte, num_validators int) *blockVotes {
        return &VotesAggr{
		Height : height,
		Round : round,
		Type : type,
...
func (va *VotesAggr) addVerifiedVote(vote *Vote, votingPower int64) {
...
// Split it into parts
func (va *VotesAggr) MakePartSet(partSize int) *PartSet {
        return NewPartSetFromData(wire.BinaryBytes(va), partSize)
}

// Split it into parts
func (va *VotesAggr) AddVotes(votes []*Vote) {
        for i := 0; i < numValidators; i++ {
		if votes[i] != ni {
			// Use voting power 0 temproralily
			addVerifiedVote(votes[i], 0)
		}
        }    
}
