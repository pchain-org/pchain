...
func NewHeightVoteSignAggr(chainID string, height int, valSet *types.ValidatorSet) *HeightVoteSignAggr {
...
func (hvs *HeightVoteSignAggr) Reset(height int, valSet *types.ValidatorSet) {
	hvs.mtx.Lock()
...
func (hvs *HeightVoteSignAggr) Height() int {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
	return hvs.height
}
