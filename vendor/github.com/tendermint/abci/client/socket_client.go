...
RETRY_LOOP:
	for {
		...
		if err != nil {
			...
			logger.Warn("abci.socketClient failed to connect to ", cli.addr, "  Retrying...")
...
func (cli *socketClient) StopForError(err error) {
	...
	logger.Warn("Stopping abci.socketClient for error: ", err.Error())
	cli.Stop()
}
...
func (cli *socketClient) sendRequestsRoutine(conn net.Conn) {
	...
	for {
		...
		case reqres := <-cli.reqQueue:
			...
			// logger.Debug("Sent request", "requestType", reflect.TypeOf(reqres.Request), "request", reqres.Request)
...
func (cli *socketClient) recvResponseRoutine(conn net.Conn) {
	...
	for {
		...
		case *types.Response_Exception:
			...
			return
		default:
			// logger.Debug("Received response", "responseType", reflect.TypeOf(res), "response", res)
...
func (cli *socketClient) EchoAsync(msg string) *ReqRes {
	return cli.queueRequest(types.ToRequestEcho(msg), nil)
}

func (cli *socketClient) FlushAsync() *ReqRes {
	return cli.queueRequest(types.ToRequestFlush(), nil)
}

func (cli *socketClient) InfoAsync() *ReqRes {
	return cli.queueRequest(types.ToRequestInfo(), nil)
}

func (cli *socketClient) SetOptionAsync(key string, value string) *ReqRes {
	return cli.queueRequest(types.ToRequestSetOption(key, value), nil)
}

func (cli *socketClient) DeliverTxAsync(tx []byte) *ReqRes {
	return cli.queueRequest(types.ToRequestDeliverTx(tx), nil)
}

func (cli *socketClient) CheckTxAsync(tx []byte, cb func(*types.Response)) *ReqRes {
	return cli.queueRequest(types.ToRequestCheckTx(tx), cb)
}
...
func (cli *socketClient) InitChainAsync(validators []*types.Validator) *ReqRes {
	return cli.queueRequest(types.ToRequestInitChain(validators), nil)
}

func (cli *socketClient) BeginBlockAsync(hash []byte, header *types.Header) *ReqRes {
	return cli.queueRequest(types.ToRequestBeginBlock(hash, header), nil)
}

func (cli *socketClient) EndBlockAsync(height uint64) *ReqRes {
	return cli.queueRequest(types.ToRequestEndBlock(height), nil)
}
...
func (cli *socketClient) InfoSync() (resInfo types.ResponseInfo, err error) {
	reqres := cli.queueRequest(types.ToRequestInfo(), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return resInfo, err
	}
...
func (cli *socketClient) SetOptionSync(key string, value string) (res types.Result) {
	reqres := cli.queueRequest(types.ToRequestSetOption(key, value), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return types.ErrInternalError.SetLog(err.Error())
	}
...
func (cli *socketClient) DeliverTxSync(tx []byte) (res types.Result) {
	reqres := cli.queueRequest(types.ToRequestDeliverTx(tx), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return types.ErrInternalError.SetLog(err.Error())
	}
...
func (cli *socketClient) CheckTxSync(tx []byte) (res types.Result) {
	reqres := cli.queueRequest(types.ToRequestCheckTx(tx), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return types.ErrInternalError.SetLog(err.Error())
	}
...
func (cli *socketClient) QuerySync(reqQuery types.RequestQuery) (resQuery types.ResponseQuery, err error) {
	reqres := cli.queueRequest(types.ToRequestQuery(reqQuery), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return resQuery, err
	}
...
func (cli *socketClient) InitChainSync(validators []*types.Validator) (err error) {
	cli.queueRequest(types.ToRequestInitChain(validators), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return err
	}
...
func (cli *socketClient) BeginBlockSync(hash []byte, header *types.Header) (err error) {
	cli.queueRequest(types.ToRequestBeginBlock(hash, header), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
		return err
	}
...
func (cli *socketClient) EndBlockSync(height uint64) (resEndBlock types.ResponseEndBlock, err error) {
	reqres := cli.queueRequest(types.ToRequestEndBlock(height), nil)
	cli.FlushSync()
	if err := cli.Error(); err != nil {
...
func (cli *socketClient) queueRequest(req *types.Request, cb func(*types.Response)) *ReqRes {
	reqres := NewReqRes(req)

	/*if _, ok := reqres.Request.Value.(*types.Request_CheckTx); ok {
		if cb != nil {
			reqres.SetCallback(cb)
		}
	} else { // check other request types

	}*/

	// To fix bug, ensure that SetCallback() is called before GetCallback() is called,
	// otherwise GetCallback() may be called earlier within another go-routine, retrieving a nil callback,
	// which will lead to a blocking in current go-routine that is waiting to read a response channel,
	// which is expected to be writen to in the callback.
	if cb != nil {
		reqres.SetCallback(cb)
	}
