...
RETRY_LOOP:
	...
	for {
		...
		if err != nil {
			...
			logger.Warn("abci.grpcClient failed to connect to ", cli.addr, " Retrying...")
...
func (cli *grpcClient) StopForError(err error) {
	...
	logger.Warn("Stopping abci.grpcClient for error:", err.Error())
	cli.Stop()
}
...
func (cli *grpcClient) CheckTxAsync(tx []byte, cb func(*types.Response)) *ReqRes {
...
func (cli *grpcClient) CheckTxSync(tx []byte) (res types.Result) {
	reqres := cli.CheckTxAsync(tx, nil)
	if res := cli.checkErrGetResult(); res.IsErr() {
		return res
	}
