...
func (sb *backend) accumulateRewardsPatch1(state *state.StateDB, header *types.Header, ep *epoch.Epoch,
	coinbase common.Address, coinbaseReward, totalGasFee *big.Int, outsideReward, selfRetrieveReward, rollbackCatchup bool) {
	...
	sb.divideRewardByEpochPatch1(state, coinbase, ep.Number, height, selfReward, outsideReward, selfRetrieveReward, rollbackCatchup)
	...
	if delegateReward != nil && delegateReward.Sign() > 0 {
		...
		state.ForEachProxied(coinbase, func(key common.Address, proxiedBalance, depositProxiedBalance, pendingRefundBalance *big.Int) bool {
			if depositProxiedBalance.Sign() == 1 {
				...
				sb.divideRewardByEpochPatch1(state, key, ep.Number, height,individualReward, outsideReward, selfRetrieveReward, rollbackCatchup)
...
func (sb *backend) accumulateRewardsPatch2(state *state.StateDB, header *types.Header, ep *epoch.Epoch,
	...
	height := header.Number.Uint64()
	sb.divideRewardByEpochPatch2(state, coinbase, ep.Number, height, selfReward, outsideReward, selfRetrieveReward, rollbackCatchup)
	...
	if delegateReward != nil && delegateReward.Sign() > 0 {
		...
		state.ForEachProxied(coinbase, func(key common.Address, proxiedBalance, depositProxiedBalance, pendingRefundBalance *big.Int) bool {
			if depositProxiedBalance.Sign() == 1 {
				// deposit * delegateReward / total deposit
				individualReward := new(big.Int).Quo(new(big.Int).Mul(depositProxiedBalance, delegateReward), totalProxiedDeposit)
				sb.divideRewardByEpochPatch2(state, key, ep.Number, height,individualReward, outsideReward, selfRetrieveReward, rollbackCatchup)
				totalIndividualReward.Add(totalIndividualReward, individualReward)
			}
			return true
		})
		...
		if cmp == 1 {
			...
			if outsideReward {
				if !rollbackCatchup {
					state.AddOutsideRewardBalanceByEpochNumberBase(coinbase, ep.Number, height, diff)
				}
			} else {
				log.Infof("should not be here")
			}
			...
			if outsideReward {
				if !rollbackCatchup {
					state.SubOutsideRewardBalanceByEpochNumberBase(coinbase, ep.Number, height, diff)
				}
			} else {
				log.Infof("should not be here")
			}
...
func (sb *backend) divideRewardByEpochPatch1(state *state.StateDB, addr common.Address, epochNumber uint64,height uint64, reward *big.Int,
...
func (sb *backend) divideRewardByEpochPatch2(state *state.StateDB, addr common.Address, epochNumber uint64,height uint64, reward *big.Int,
								outsideReward, selfRetrieveReward, rollbackCatchup bool) {
	...
	for i := epochNumber; i < epochNumber+12; i++ {
		if i == epochNumber+11 {
			if outsideReward {
				if !rollbackCatchup {
					state.AddOutsideRewardBalanceByEpochNumberBase(addr, i, height, lastEpochReward)
				}
			} else {
				log.Infof("should not be here")
			}
			...
			if outsideReward {
				if !rollbackCatchup {
					state.AddOutsideRewardBalanceByEpochNumberBase(addr, i, height, epochReward)
				}
			} else {
				log.Infof("should not be here")
			}
