...
func (self *peer) start() {
	go self.update()
	logger.Debugf("%v: whisper started", self.peer)
}

// stop terminates the peer updater, stopping message forwarding to it.
func (self *peer) stop() {
	close(self.quit)
	logger.Debugf("%v: whisper stopped", self.peer)
}
	...
	for {
		...
		case <-transmit.C:
			if err := self.broadcast(); err != nil {
				logger.Infof("%v: broadcast failed: %v", self.peer, err)
				return
			}
...
func (self *peer) broadcast() error {
	...
	logger.Debug(self.peer, "broadcasted", len(transmit), "message(s)")
	return nil
}
