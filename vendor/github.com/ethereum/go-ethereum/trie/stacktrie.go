...
func stackTrieFromPool(db ethdb.Writer) *StackTrie {
...
type StackTrie struct {
	...
	db        ethdb.Writer // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db ethdb.Writer) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewFromBinary(data []byte, db ethdb.Writer) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db ethdb.Writer) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(ko int, key, val []byte, db ethdb.Writer) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db ethdb.Writer) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
