...
func (eth *Ethereum) stateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool) (statedb *state.StateDB, err error) {
	...
	if checkLive {
		statedb, err = eth.blockchain.StateAtHeader(block.Header())
		if err == nil {
			return statedb, nil
		}
	}
...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	signer := types.MakeSignerWithMainBlock(eth.blockchain.Config(), block.Header().MainChainNumber)
	for idx, tx := range block.Transactions() {
		...
		if _, _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(tx.Gas()), nil); err != nil {
