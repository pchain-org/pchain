...
func (q *queue) Schedule(headers []*types.Header, from uint64) []*types.Header {
	...
	for _, header := range headers {
		...
		if header.Number == nil || header.Number.Uint64() != from {
			logger.Warnf("Header #%v [%x…] broke chain ordering, expected %d", header.Number, hash[:4], from)
			break
		}
		if q.headerHead != (common.Hash{}) && q.headerHead != header.ParentHash {
			logger.Warnf("Header #%v [%x…] broke chain ancestry", header.Number, hash[:4])
			break
		}
		// Make sure no duplicate requests are executed
		if _, ok := q.blockTaskPool[hash]; ok {
			logger.Warnf("Header #%d [%x…] already scheduled for block fetch", header.Number.Uint64(), hash[:4])
			continue
		}
		if _, ok := q.receiptTaskPool[hash]; ok {
			logger.Warnf("Header #%d [%x…] already scheduled for receipt fetch", header.Number.Uint64(), hash[:4])
			continue
		}
		...
		if q.mode == FastSync && header.Number.Uint64() == q.fastSyncPivot {
			...
			logger.Debugf("Switching state downloads to %d [%x…]", header.Number.Uint64(), header.Hash().Bytes()[:4])
...
func (q *queue) DeliverHeaders(id string, headers []*types.Header, headerProcCh chan []*types.Header) (int, error) {
	...
	if accepted {
		if headers[0].Number.Uint64() != request.From {
			logger.Debugf("Peer %s: first header #%v [%x…] broke chain ordering, expected %d", id, headers[0].Number, headers[0].Hash().Bytes()[:4], request.From)
			accepted = false
		} else if headers[len(headers)-1].Hash() != target {
			logger.Debugf("Peer %s: last header #%v [%x…] broke skeleton structure, expected %x", id, headers[len(headers)-1].Number, headers[len(headers)-1].Hash().Bytes()[:4], target[:4])
			accepted = false
		}
	}
	if accepted {
		for i, header := range headers[1:] {
			...
			if want := request.From + 1 + uint64(i); header.Number.Uint64() != want {
				logger.Warnf("Peer %s: header #%v [%x…] broke chain ordering, expected %d", id, header.Number, hash[:4], want)
				accepted = false
				break
			}
			if headers[i].Hash() != header.ParentHash {
				logger.Warnf("Peer %s: header #%v [%x…] broke chain ancestry", id, header.Number, hash[:4])
				accepted = false
				break
			}
		}
	...
	if !accepted {
		logger.Debugf("Peer %s: skeleton filling from header #%d not accepted", id, request.From)
	...
	if ready > 0 {
		...
		case headerProcCh <- process:
			logger.Debugf("%s: pre-scheduled %d headers from #%v", id, len(process), process[0].Number)
