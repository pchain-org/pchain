...
func (d *Downloader) RegisterPeer(id string, version int, currentHead currentHeadRetrievalFn,
	...
	logger.Debugf("Registering peer", id)
...
func (d *Downloader) UnregisterPeer(id string) error {
	...
	logger.Debugf("Unregistering peer", id)
	if err := d.peers.Unregister(id); err != nil {
		logger.Error("Unregister failed:", err)
		return err
	}
	d.queue.Revoke(id)
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	logger.Debugf("Attempting synchronisation: %v, head [%x…], TD %v", id, head[:4], td)

	err := d.synchronise(id, head, td, mode)
	switch err {
	case nil:
		logger.Debugf("Synchronisation completed")

	case errBusy:
		logger.Debugf("Synchronisation already in progress")

	case errTimeout, errBadPeer, errStallingPeer,
		errEmptyHeaderSet, errPeersUnavailable, errTooOld,
		errInvalidAncestor, errInvalidChain:
		logger.Debugf("Removing peer %v: %v", id, err)
		d.dropPeer(id)

	default:
		logger.Warnf("Synchronisation failed: %v", err)
	}
	return err
}
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	logger.Debugf("Synchronising with the network using: %s [eth/%d]", p.id, p.version)
	defer func(start time.Time) {
		logger.Debugf("Synchronisation terminated after %v", time.Since(start))
	}(time.Now())
	...
	case FastSync:
		...
		logger.Debugf("Fast syncing until pivot block #%d", pivot)
	}
...
func (d *Downloader) fetchHeight(p *peer) (*types.Header, error) {
	logger.Debugf("%v: retrieving remote chain height", p)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if packet.PeerId() != p.id {
				logger.Debugf("Received headers from incorrect peer(%s)", packet.PeerId())
				break
			}
			// Make sure the peer actually gave something valid
			headers := packet.(*headerPack).headers
			if len(headers) != 1 {
				logger.Debugf("%v: invalid number of head headers: %d != 1", p, len(headers))
				return nil, errBadPeer
			}
			return headers[0], nil

		case <-timeout:
			logger.Debugf("%v: head header timeout", p)
...
func (d *Downloader) findAncestor(p *peer, height uint64) (uint64, error) {
	logger.Debugf("%v: looking for common ancestor (remote height %d)", p, height)
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
				logger.Warnf("%v: empty head header set", p)
				return 0, errEmptyHeaderSet
			}
			// Make sure the peer's reply conforms to the request
			for i := 0; i < len(headers); i++ {
				if number := headers[i].Number.Int64(); number != from+int64(i)*16 {
					logger.Warnf("%v: head header set (item %d) broke chain ordering: requested %d, got %d", p, i, from+int64(i)*16, number)
			...
			for i := len(headers) - 1; i >= 0; i-- {
				...
				if (d.mode == FullSync && d.hasBlockAndState(headers[i].Hash())) || (d.mode != FullSync && d.hasHeader(headers[i].Hash())) {
					...
					if number > height && i == limit-1 {
						logger.Warnf("%v: lied about chain head: reported %d, found above %d", p, height, number)
	...
	if !common.EmptyHash(hash) {
		if int64(number) <= floor {
			logger.Warnf("%v: potential rewrite attack: #%d [%x…] <= #%d limit", p, number, hash[:4], floor)
			return 0, errInvalidAncestor
		}
		logger.Debugf("%v: common ancestor: #%d [%x…]", p, number, hash[:4])
		return number, nil
	}
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.headerCh:
				...
				if packer.PeerId() != p.id {
					logger.Debugf("Received headers from incorrect peer(%s)", packer.PeerId())
					break
				}
				// Make sure the peer actually gave something valid
				headers := packer.(*headerPack).headers
				if len(headers) != 1 {
					logger.Debugf("%v: invalid search header set (%d)", p, len(headers))
					return 0, errBadPeer
				}
				arrived = true
				...
				if header.Number.Uint64() != check {
					logger.Debugf("%v: non requested header #%d [%x…], instead of #%d", p, header.Number, header.Hash().Bytes()[:4], check)
					return 0, errBadPeer
				}
				start = check

			case <-timeout:
				logger.Debugf("%v: search header timeout", p)
				return 0, errTimeout
	...
	if int64(start) <= floor {
		logger.Warnf("%v: potential rewrite attack: #%d [%x…] <= #%d limit", p, start, hash[:4], floor)
		return 0, errInvalidAncestor
	}
	logger.Debugf("%v: common ancestor: #%d [%x…]", p, start, hash[:4])
	return start, nil
}
...
func (d *Downloader) fetchHeaders(p *peer, from uint64) error {
	logger.Debugf("%v: directing header downloads from #%d", p, from)
	defer logger.Debugf("%v: header download terminated", p)
	...
	getHeaders := func(from uint64) {
		...
		if skeleton {
			logger.Debugf("%v: fetching %d skeleton headers from #%d", p, MaxHeaderFetch, from)
			go p.getAbsHeaders(from+uint64(MaxHeaderFetch)-1, MaxSkeletonSize, MaxHeaderFetch-1, false)
		} else {
			logger.Debugf("%v: fetching %d full headers from #%d", p, MaxHeaderFetch, from)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if packet.PeerId() != p.id {
				logger.Debugf("Received skeleton headers from incorrect peer (%s)", packet.PeerId())
				break
			}
			...
			if packet.Items() == 0 {
				logger.Debugf("%v: no available headers", p)
				select {
			...
			if skeleton {
				...
				if err != nil {
					logger.Debugf("%v: skeleton chain invalid: %v", p, err)
			...
			if len(headers) > 0 {
				logger.Debugf("%v: schedule %d headers from #%d", p, len(headers), from)
				select {
		...
		case <-timeout.C:
			...
			logger.Debugf("%v: header request timed out", p)
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, int, error) {
	logger.Debugf("Filling up skeleton from #%d", from)
	...
	logger.Debugf("Skeleton fill terminated: %v", err)
...
func (d *Downloader) fetchBodies(from uint64) error {
	logger.Debugf("Downloading block bodies from #%d", from)
	...
	logger.Debugf("Block body download terminated: %v", err)
	return err
}
...
func (d *Downloader) fetchReceipts(from uint64) error {
	logger.Debugf("Downloading receipts from #%d", from)
	...
	logger.Debugf("Receipt download terminated: %v", err)
	return err
}
...
func (d *Downloader) fetchNodeData() error {
	logger.Debugf("Downloading node state data")
		...
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverNodeData(packet.PeerId(), packet.(*statePack).states, func(delivered int, progressed bool, err error) {
				...
				if err == trie.ErrNotRequested {
					logger.Debugf("peer %s: replied to stale state request, forgiving", packet.PeerId())
					return
				}
				if err != nil {
					// If the node data processing failed, the root hash is very wrong, abort
					logger.Errorf("peer %s: state processing failed: %v", packet.PeerId(), err)
					d.cancel()
					return
				}
				...
				if progressed && atomic.LoadUint32(&d.fsPivotFails) > 1 {
					logger.Debugf("fast-sync progressed, resetting fail counter from %d", atomic.LoadUint32(&d.fsPivotFails))
					atomic.StoreUint32(&d.fsPivotFails, 1) // Don't ever reset to 0, as that will unlock the pivot block
				}
				// Log a message to the user and return
				if delivered > 0 {
					logger.Infof("imported %3d state entries in %9v: processed %d, pending at least %d", delivered, common.PrettyDuration(time.Since(start)), syncStatsStateDone, pending)
				}
			})
		}
	...
	logger.Debugf("Node state data download terminated: %v", err)
	return err
}
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				case err == nil && packet.Items() == 0:
					logger.Debugf("%s: no %s delivered", peer, strings.ToLower(kind))
				case err == nil:
					logger.Debugf("%s: delivered %s %s(s)", peer, packet.Stats(), strings.ToLower(kind))
				default:
					logger.Debugf("%s: %s delivery failed: %v", peer, strings.ToLower(kind), err)
				}
			}
		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := d.peers.Peer(pid); peer != nil {
					...
					if fails > 2 {
						logger.Debugf("%s: %s delivery timeout", peer, strings.ToLower(kind))
						setIdle(peer, 0)
					} else {
						logger.Debugf("%s: stalling %s delivery, dropping", peer, strings.ToLower(kind))
						d.dropPeer(pid)
					}
				}
			}
			...
			if pending() == 0 {
				if !inFlight() && finished {
					logger.Debugf("%s fetching completed", kind)
					return nil
				}
				break
			}
			...
			for _, peer := range idles {
				...
				if logger.Level >= logrus.DebugLevel {
					if request.From > 0 {
						logger.Debugf("%s: requesting %s(s) from #%d", peer, strings.ToLower(kind), request.From)
					} else if len(request.Headers) > 0 {
						logger.Debugf("%s: requesting %d %s(s), first at #%d", peer, len(request.Headers), strings.ToLower(kind), request.Headers[0].Number)
					} else {
						logger.Debugf("%s: requesting %d %s(s)", peer, len(request.Hashes), strings.ToLower(kind))
					}
				}
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			logger.Warnf("Rolled back %d headers (LH: %d->%d, FB: %d->%d, LB: %d->%d)",
				...
				if atomic.LoadUint32(&d.fsPivotFails) == 0 {
					for _, header := range rollback {
						if header.Number.Uint64() == pivot {
							logger.Warnf("Fast-sync critical section failure, locked pivot to header #%d [%x…]", pivot, header.Hash().Bytes()[:4])
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					if n, err := d.insertHeaders(chunk, frequency); err != nil {
						...
						logger.Debugf("invalid header #%d [%x…]: %v", chunk[n].Number, chunk[n].Hash().Bytes()[:4], err)
						return errInvalidChain
					}
				...
				if d.mode == FastSync && d.fsPivotLock != nil && chunk[0].Number.Uint64() <= pivot && chunk[len(chunk)-1].Number.Uint64() >= pivot {
					if pivot := chunk[int(pivot-chunk[0].Number.Uint64())]; pivot.Hash() != d.fsPivotLock.Hash() {
						logger.Warnf("Pivot doesn't match locked in version: have #%v [%x…], want #%v [%x…]", pivot.Number, pivot.Hash().Bytes()[:4], d.fsPivotLock.Number, d.fsPivotLock.Hash().Bytes()[:4])
						return errInvalidChain
					}
				}
...
func (d *Downloader) processContent() error {
	...
	for {
		...
		if logger.Level >= logrus.DebugLevel {
			...
			logger.Debugf("Inserting chain with %d items (#%d [%x…] - #%d [%x…])", len(results), first.Number, first.Hash().Bytes()[:4], last.Number, last.Hash().Bytes()[:4])
		}
		for len(results) != 0 {
			...
			case len(receipts) > 0:
				...
				if err == nil && blocks[len(blocks)-1].NumberU64() == pivot {
					logger.Debugf("Committing block #%d [%x…] as the new head", blocks[len(blocks)-1].Number(), blocks[len(blocks)-1].Hash().Bytes()[:4])
			...
			if err != nil {
				logger.Debugf("Result #%d [%x…] processing failed: %v", results[index].Header.Number, results[index].Header.Hash().Bytes()[:4], err)
				return errInvalidChain
			}
...
func (d *Downloader) qosTuner() {
	for {
		...
		logger.Debugf("Quality of service: rtt %v, conf %.3f, ttl %v", rtt, float64(conf)/1000000.0, d.requestTTL())
		select {
		case <-d.quitCh:
			return
		case <-time.After(rtt):
