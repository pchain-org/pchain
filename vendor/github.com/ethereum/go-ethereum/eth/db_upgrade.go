...
func upgradeSequentialKeys(db ethdb.Database) (stopFn func()) {
	...
	logger.Infof("Upgrading chain database to use sequential keys")
	...
	go func() {
		...
		if err == nil && !stopped {
			logger.Infof("Database conversion successful")
			db.Put(useSequentialKeys, []byte{42})
		}
		if err != nil {
			logger.Errorf("Database conversion failed: %v", err)
		}
		close(stoppedChn)
	}()
...
func upgradeSequentialCanonicalNumbers(db ethdb.Database, stopFn func() bool) (error, bool) {
	...
	for bytes.HasPrefix(it.Key(), prefix) {
		...
		if len(keyPtr) < 20 {
			...
			if cnt%100000 == 0 {
				...
				it = db.(*ethdb.LDBDatabase).NewIterator()
				it.Seek(keyPtr)
				logger.Infof("converting %d canonical numbers...", cnt)
			}
	...
	if cnt > 0 {
		logger.Infof("converted %d canonical numbers...", cnt)
	}
	return nil, false
}
...
func upgradeSequentialBlocks(db ethdb.Database, stopFn func() bool) (error, bool) {
	...
	for bytes.HasPrefix(it.Key(), prefix) {
		...
		if len(keyPtr) >= 38 {
			...
			if cnt%10000 == 0 {
				...
				it = db.(*ethdb.LDBDatabase).NewIterator()
				it.Seek(keyPtr)
				logger.Infof("converting %d blocks...", cnt)
			}
	...
	if cnt > 0 {
		logger.Infof("converted %d blocks...", cnt)
	}
	return nil, false
}
...
func upgradeSequentialOrphanedReceipts(db ethdb.Database, stopFn func() bool) (error, bool) {
	...
	if cnt > 0 {
		logger.Infof("removed %d orphaned block receipts...", cnt)
	}
	return nil, false
}
...
func upgradeChainDatabase(db ethdb.Database) error {
	...
	logger.Infof("Old database detected, upgrading...")
...
func addMipmapBloomBins(db ethdb.Database) (err error) {
	...
	tstart := time.Now()
	logger.Info("upgrading db log bloom bins")
	...
	logger.Info("upgrade completed in", time.Since(tstart))
	return nil
}
