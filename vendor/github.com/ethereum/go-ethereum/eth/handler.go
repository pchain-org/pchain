...
func NewProtocolManager(config *params.ChainConfig, fastSync bool, networkId int, maxPeers int, mux *event.TypeMux, txpool txPool, pow pow.PoW, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	...
	if fastSync && blockchain.CurrentBlock().NumberU64() > 0 {
		logger.Infof("blockchain not empty, fast sync disabled")
		fastSync = false
	}
	if fastSync {
	...
	if blockchain.Genesis().Hash().Hex() == defaultGenesisHash && networkId == 1 {
		logger.Debugf("Bad Block Reporting is enabled")
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	if err := pm.peers.Unregister(id); err != nil {
		logger.Errorf("Removal failed:", err)
	}
...
func (pm *ProtocolManager) Stop() {
	logger.Info("Stopping ethereum protocol handler...")
	...
	logger.Info("Ethereum protocol handler stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if daoBlock := pm.chainconfig.DAOForkBlock; daoBlock != nil {
		...
		p.forkDrop = time.AfterFunc(daoChallengeTimeout, func() {
			logger.Debugf("%v: timed out DAO fork-check, dropping", p)
			pm.removePeer(p.id)
		})
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader.MaxHeaderFetch {
			...
			case query.Origin.Hash != (common.Hash{}) && !query.Reverse:
				...
				if next <= current {
					...
					logger.Warnf("%v: GetBlockHeaders skip overflow attack (current %v, skip %v, next %v)\nMalicious peer infos: %s", p, current, query.Skip, next, infos)
					unknown = true
				} else {
	...
	case msg.Code == BlockHeadersMsg:
		...
		if len(headers) == 0 && p.forkDrop != nil {
			...
			if verifyDAO {
				logger.Debugf("%v: seems to be on the same side of the DAO fork", p)
				p.forkDrop.Stop()
				p.forkDrop = nil
				return nil
			}
		...
		if filter {
			...
			if p.forkDrop != nil && pm.chainconfig.DAOForkBlock.Cmp(headers[0].Number) == 0 {
				...
				if err := core.ValidateDAOHeaderExtraData(pm.chainconfig, headers[0]); err != nil {
					logger.Debugf("%v: verified to be on the other side of the DAO fork, dropping", p)
					return err
				}
				logger.Debugf("%v: verified to be on the same side of the DAO fork", p)
				return nil
			}
	...
	case p.version >= eth63 && msg.Code == NodeDataMsg:
		...
		if err := pm.downloader.DeliverNodeData(p.id, data); err != nil {
			logger.Debugf("failed to deliver node state data: %v", err)
		}
	...
	case p.version >= eth63 && msg.Code == ReceiptsMsg:
		...
		if err := pm.downloader.DeliverReceipts(p.id, receipts); err != nil {
			logger.Debugf("failed to deliver receipts: %v", err)
		}
...
func (pm *ProtocolManager) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		if parent := pm.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1); parent != nil {
			...
			logger.Errorf("propagating dangling block #%d [%x]", block.NumberU64(), hash[:4])
			return
		}
		...
		logger.Debugf("propagated block %x to %d peers in %v", hash[:4], len(transfer), time.Since(block.ReceivedAt))
	}
	...
	if pm.blockchain.HasBlock(hash) {
		...
		logger.Debugf("announced block %x to %d peers in %v", hash[:4], len(peers), time.Since(block.ReceivedAt))
	}
}
...
func (pm *ProtocolManager) BroadcastTx(hash common.Hash, tx *types.Transaction) {
	...
	logger.Debugf("broadcast tx to", len(peers), "peers")
}
