...
func (p *peer) RequestHeadersByHash(reqID, cost uint64, origin common.Hash, amount int, skip int, reverse bool) error {
	logger.Debugf("%v fetching %d headers from %x, skipping %d (reverse = %v)", p, amount, origin[:4], skip, reverse)
...
func (p *peer) RequestHeadersByNumber(reqID, cost, origin uint64, amount int, skip int, reverse bool) error {
	logger.Debugf("%v fetching %d headers from #%d, skipping %d (reverse = %v)", p, amount, origin, skip, reverse)
...
func (p *peer) RequestBodies(reqID, cost uint64, hashes []common.Hash) error {
	logger.Debugf("%v fetching %d block bodies", p, len(hashes))
...
func (p *peer) RequestHeaderProofs(reqID, cost uint64, reqs []*ChtReq) error {
	logger.Debugf("%v fetching %v header proofs", p, len(reqs))
	return sendRequest(p.rw, GetHeaderProofsMsg, reqID, cost, reqs)
}

func (p *peer) SendTxs(cost uint64, txs types.Transactions) error {
	logger.Debugf("%v relaying %v txs", p, len(txs))
	reqID := getNextReqID()
