...
func (self *BlockRequest) Request(reqID uint64, peer *peer) error {
	logger.Debugf("ODR: requesting body of block %08x from peer %v", self.Hash[:4], peer.id)
...
func (self *BlockRequest) Valid(db ethdb.Database, msg *Msg) bool {
	logger.Debugf("ODR: validating body of block %08x", self.Hash[:4])
	if msg.MsgType != MsgBlockBodies {
		logger.Debugf("ODR: invalid message type")
		return false
	}
	bodies := msg.Obj.([]*types.Body)
	if len(bodies) != 1 {
		logger.Debugf("ODR: invalid number of entries: %d", len(bodies))
		return false
	}
	body := bodies[0]
	header := core.GetHeader(db, self.Hash, self.Number)
	if header == nil {
		logger.Debugf("ODR: header not found for block %08x", self.Hash[:4])
		return false
	}
	txHash := types.DeriveSha(types.Transactions(body.Transactions))
	if header.TxHash != txHash {
		logger.Debugf("ODR: header.TxHash %08x does not match received txHash %08x", header.TxHash[:4], txHash[:4])
		return false
	}
	uncleHash := types.CalcUncleHash(body.Uncles)
	if header.UncleHash != uncleHash {
		logger.Debugf("ODR: header.UncleHash %08x does not match received uncleHash %08x", header.UncleHash[:4], uncleHash[:4])
		return false
	}
	data, err := rlp.EncodeToBytes(body)
	if err != nil {
		logger.Debugf("ODR: body RLP encode error: %v", err)
		return false
	}
	self.Rlp = data
	logger.Debugf("ODR: validation successful")
	return true
}
...
func (self *ReceiptsRequest) Request(reqID uint64, peer *peer) error {
	logger.Debugf("ODR: requesting receipts for block %08x from peer %v", self.Hash[:4], peer.id)
...
func (self *ReceiptsRequest) Valid(db ethdb.Database, msg *Msg) bool {
	logger.Debugf("ODR: validating receipts for block %08x", self.Hash[:4])
	...
	if len(receipts) != 1 {
		logger.Debugf("ODR: invalid number of entries: %d", len(receipts))
		return false
	}
	...
	if !bytes.Equal(header.ReceiptHash[:], hash[:]) {
		logger.Debugf("ODR: header receipts hash %08x does not match calculated RLP hash %08x", header.ReceiptHash[:4], hash[:4])
		return false
	}
...
func (self *TrieRequest) Request(reqID uint64, peer *peer) error {
	logger.Debugf("ODR: requesting trie root %08x key %08x from peer %v", self.Id.Root[:4], self.Key[:4], peer.id)
	req := &ProofReq{
...
func (self *TrieRequest) Valid(db ethdb.Database, msg *Msg) bool {
	logger.Debugf("ODR: validating trie root %08x key %08x", self.Id.Root[:4], self.Key[:4])
	...
	if len(proofs) != 1 {
		logger.Debugf("ODR: invalid number of entries: %d", len(proofs))
		return false
	}
	_, err := trie.VerifyProof(self.Id.Root, self.Key, proofs[0])
	if err != nil {
		logger.Debugf("ODR: merkle proof verification error: %v", err)
		return false
	}
	self.Proof = proofs[0]
	logger.Debugf("ODR: validation successful")
...
func (self *CodeRequest) Request(reqID uint64, peer *peer) error {
	logger.Debugf("ODR: requesting node data for hash %08x from peer %v", self.Hash[:4], peer.id)
	req := &CodeReq{
		BHash:  self.Id.BlockHash,
		AccKey: self.Id.AccKey,
	}
...
func (self *CodeRequest) Valid(db ethdb.Database, msg *Msg) bool {
	logger.Debugf("ODR: validating node data for hash %08x", self.Hash[:4])
	...
	reply := msg.Obj.([][]byte)
	if len(reply) != 1 {
		logger.Debugf("ODR: invalid number of entries: %d", len(reply))
		return false
	}
	data := reply[0]
	if hash := crypto.Keccak256Hash(data); self.Hash != hash {
		logger.Debugf("ODR: requested hash %08x does not match received data hash %08x", self.Hash[:4], hash[:4])
		return false
	}
	self.Data = data
	logger.Debugf("ODR: validation successful")
...
func (self *ChtRequest) Request(reqID uint64, peer *peer) error {
	logger.Debugf("ODR: requesting CHT #%d block #%d from peer %v", self.ChtNum, self.BlockNum, peer.id)
	req := &ChtReq{
		ChtNum:   self.ChtNum,
...
func (self *ChtRequest) Valid(db ethdb.Database, msg *Msg) bool {
	logger.Debugf("ODR: validating CHT #%d block #%d", self.ChtNum, self.BlockNum)
	...
	if err != nil {
		logger.Debugf("ODR: CHT merkle proof verification error: %v", err)
		return false
	}
	var node light.ChtNode
	if err := rlp.DecodeBytes(value, &node); err != nil {
		logger.Debugf("ODR: error decoding CHT node: %v", err)
		return false
	}
	if node.Hash != proof.Header.Hash() {
		logger.Debugf("ODR: CHT header hash does not match")
		return false
	}
