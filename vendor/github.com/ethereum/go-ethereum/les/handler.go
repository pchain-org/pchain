...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId int, mux *event.TypeMux, pow pow.PoW, blockchain BlockChain, txpool txPool, chainDb ethdb.Database, odr *LesOdr, txrelay *LesTxRelay) (*ProtocolManager, error) {
	...
	if lightSync {
		logger.Debugf("LES: create downloader")
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	logger.Debugf("Removing peer", id)

	// Unregister the peer from the downloader and Ethereum peer set
	logger.Debugf("LES: unregister peer %v", id)
	if pm.lightSync {
...
func (pm *ProtocolManager) Start(srvr *p2p.Server) {
	...
	if pm.lightSync {
		...
		if topicDisc != nil {
			go func() {
				logger.Info("Starting registering topic", string(lesTopic))
				topicDisc.RegisterTopic(lesTopic, pm.quitSync)
				logger.Info("Stopped registering topic", string(lesTopic))
			}()
		}
		go func() {
...
func (pm *ProtocolManager) Stop() {
	...
	logger.Info("Stopping light ethereum protocol handler...")
	...
	logger.Info("Light ethereum protocol handler stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	logger.Debugf("%v: adding peer", p)
	if err := pm.peers.Register(p); err != nil {
		logger.Errorf("%v: addition failed: %v", p, err)
		return err
	}
	defer func() {
	...
	logger.Debugf("LES: register peer %v", p.id)
	if pm.lightSync {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			logger.Debugf("%v: message handling failed: %v", p, err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	logger.Debugf("msg:", msg.Code, msg.Size)
	...
	reject := func(reqCnt, maxCnt uint64) bool {
		...
		if cost > bufValue {
			logger.Errorf("Request from %v came %v too early", p.id, time.Duration((cost-bufValue)*1000000/pm.server.defParams.MinRecharge))
			return true
		}
		return false
	}
	...
	switch msg.Code {
	case StatusMsg:
		logger.Debugf("<=== StatusMsg from peer %v", p.id)
		// Status messages should never arrive after the handshake
		return errResp(ErrExtraStatusMsg, "uncontrolled status message")

	// Block header query, collect the requested headers and reply
	case AnnounceMsg:
		logger.Debugf("<=== AnnounceMsg from peer %v:", p.id)

		var req announceData
		if err := msg.Decode(&req); err != nil {
			return errResp(ErrDecode, "%v: %v", msg, err)
		}
		logger.Debugf("AnnounceMsg:", req.Number, req.Hash, req.Td, req.ReorgDepth)
		if pm.fetcher != nil {
			pm.fetcher.announce(p, &req)
		}

	case GetBlockHeadersMsg:
		logger.Debugf("<=== GetBlockHeadersMsg from peer %v", p.id)
	...
	case BlockHeadersMsg:
		...
		logger.Debugf("<=== BlockHeadersMsg from peer %v", p.id)
	...
	case GetBlockBodiesMsg:
		logger.Debugf("<===  GetBlockBodiesMsg from peer %v", p.id)
	...
	case BlockBodiesMsg:
		...
		logger.Debugf("<===  BlockBodiesMsg from peer %v", p.id)
	...
	case GetCodeMsg:
		logger.Debugf("<===  GetCodeMsg from peer %v", p.id)
		// Decode the retrieval message
		var req struct {
			ReqID uint64
			Reqs  []CodeReq
	...
	case CodeMsg:
		...
		logger.Debugf("<=== CodeMsg from peer %v", p.id)
	...
	case GetReceiptsMsg:
		logger.Debugf("<===  GetReceiptsMsg from peer %v", p.id)
		// Decode the retrieval message
		var req struct {
			ReqID  uint64
			Hashes []common.Hash
		...
		for _, hash := range req.Hashes {
			...
			if encoded, err := rlp.EncodeToBytes(results); err != nil {
				logger.Errorf("failed to encode receipt: %v", err)
			} else {
	...
	case ReceiptsMsg:
		...
		logger.Debugf("<=== ReceiptsMsg from peer %v", p.id)
	...
	case GetProofsMsg:
		logger.Debugf("<=== GetProofsMsg from peer %v", p.id)
		// Decode the retrieval message
		var req struct {
			ReqID uint64
			Reqs  []ProofReq
	...
	case ProofsMsg:
		...
		logger.Debugf("<=== ProofsMsg from peer %v", p.id)
	...
	case GetHeaderProofsMsg:
		logger.Debugf("<=== GetHeaderProofsMsg from peer %v", p.id)
		// Decode the retrieval message
		var req struct {
			ReqID uint64
			Reqs  []ChtReq
	...
	case HeaderProofsMsg:
		...
		logger.Debugf("<=== HeaderProofsMsg from peer %v", p.id)
		var resp struct {
			ReqID, BV uint64
			Data      []ChtResp
		}
	...
	default:
		logger.Debugf("<=== unknown message with code %d from peer %v", msg.Code, p.id)
