...
// Processor.
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, cch CrossChainHelper) (*BlockChain, error) {
	if cacheConfig == nil {
...
func SetReceiptsData(config *params.ChainConfig, block *types.Block, receipts types.Receipts) error {
	signer := types.MakeSignerWithMainBlock(config, block.Header().MainChainNumber)
	...
	if rewardOutside {
		...
		for addr, reward := range outsideReward {
			for epoch, rewardAmount := range reward {
				if rewardAmount.Sign() < 0 && ((bc.chainConfig.PChainId== params.MAINNETPCHAINID && block.NumberU64() == 13311677) ||
					(bc.chainConfig.PChainId==params.CHILD0PCHAINID && block.NumberU64()==22094435))  {
