...
// MarkMainChainToChildChainTx op
type MarkMainChainToChildChainTxOp struct {
	CrossChainTx
}

func (op *MarkMainChainToChildChainTxOp) Conflict(op1 PendingOp) bool {
	if op1, ok := op1.(*MarkMainChainToChildChainTxOp); ok {
		return op.CrossChainTx.Equal(&op1.CrossChainTx)
	}
	return false
}

func (op *MarkMainChainToChildChainTxOp) String() string {
	return fmt.Sprintf("MarkMainChainToChildChainTxOp - From: %x, ChainId: %s, TxHash: %x",
