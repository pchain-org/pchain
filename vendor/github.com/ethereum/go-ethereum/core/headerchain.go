...
func (hc *HeaderChain) WriteHeader(header *types.Header) (status WriteStatus, err error) {
	...
	if err := hc.WriteTd(hash, number, externTd); err != nil {
		 logger.Errorf("failed to write header total difficulty: %v", err)
	}
	if err := WriteHeader(hc.chainDb, header); err != nil {
		 logger.Errorf("failed to write header contents: %v", err)
	}
	...
	if externTd.Cmp(localTd) > 0 || (externTd.Cmp(localTd) == 0 && mrand.Float64() < 0.5) {
		...
		if err := WriteCanonicalHash(hc.chainDb, hash, number); err != nil {
			 logger.Errorf("failed to insert header number: %v", err)
		}
		if err := WriteHeadHeaderHash(hc.chainDb, hash); err != nil {
			 logger.Errorf("failed to insert head header hash: %v", err)
		}
...
func (hc *HeaderChain) InsertHeaderChain(chain []*types.Header, checkFreq int, writeHeader WhCallback) (int, error) {
	...
	for i, header := range chain {
		...
		if hc.procInterrupt() {
			logger.Debugf("premature abort during header chain processing")
			break
		}
		hash := header.Hash()
	...
	logger.Infof("imported %4d headers%s in %9v. #%v [%x… / %x…]", stats.processed, ignored, common.PrettyDuration(time.Since(start)), last.Number, first.Hash().Bytes()[:4], last.Hash().Bytes()[:4])
...
func (hc *HeaderChain) SetHead(head uint64, delFn DeleteCallback) {
	...
	if err := WriteHeadHeaderHash(hc.chainDb, hc.currentHeaderHash); err != nil {
		 logger.Errorf("failed to reset head header hash: %v", err)
	}
}
