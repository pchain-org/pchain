...
func newSyncDb(db *storage.LDBDatabase, key storage.Key, priority uint, bufferSize, dbBatchSize uint, deliver func(interface{}, chan bool) bool) *syncDb {
	...
	logger.Debugf("syncDb[peer: %v, priority: %v] - initialised", key.Log(), priority)
...
func (self *syncDb) bufferRead(deliver func(interface{}, chan bool) bool) {
	...
	if err == nil {
		...
		logger.Debugf("syncDb[%v/%v] - counter read from db at %v", self.key.Log(), self.priority, counter)
	} else {
		logger.Debugf("syncDb[%v/%v] - counter starts at %v", self.key.Log(), self.priority, counter)
	}
...
LOOP:
	for {
		...
		case req = <-buffer:
			...
			if !more {
				logger.Debugf("syncDb[%v/%v] quit: switching to db. session tally (db/total): %v/%v", self.key.Log(), self.priority, self.dbTotal, self.total)
			...
			self.total++
			logger.Debugf("syncDb[%v/%v] deliver (db/total): %v/%v", self.key.Log(), self.priority, self.dbTotal, self.total)
			...
			if len(buffer) == cap(buffer) {
				logger.Debugf("syncDb[%v/%v] buffer full %v: switching to db. session tally (db/total): %v/%v", self.key.Log(), self.priority, cap(buffer), self.dbTotal, self.total)
				buffer = nil
				db = self.buffer
			}
			continue LOOP
		...
		case req, more = <-db:
			if !more {
				...
				logger.Debugf("syncDb[%v/%v] quitting: save current batch to db", self.key.Log(), self.priority)
				break LOOP
			}
			self.dbTotal++
			self.total++
		...
		case dbSize = <-self.batch:
			...
			if inBatch == 0 && quit != nil {
				...
				logger.Debugf("syncDb[%v/%v] empty db: switching to buffer", self.key.Log(), self.priority)
				db = nil
				buffer = self.buffer
			...
			logger.Debugf("syncDb[%v/%v] write batch %v/%v - %x - %x", self.key.Log(), self.priority, inBatch, counter, self.counterKey, counterValue)
		...
		case <-quit:
			...
			buffer = nil
			quit = nil
			logger.Debugf("syncDb[%v/%v] quitting: save buffer to db", self.key.Log(), self.priority)
			close(db)
			continue LOOP
		}
		...
		if err != nil {
			logger.Warnf("syncDb[%v/%v] saving request %v (#%v/%v) failed: %v", self.key.Log(), self.priority, req, inBatch, inDb, err)
			continue LOOP
		}
		batch.Put(entry.key, entry.val)
		logger.Debugf("syncDb[%v/%v] to batch %v '%v' (#%v/%v/%v)", self.key.Log(), self.priority, req, entry, inBatch, inDb, counter)
		// if just switched to db mode and not quitting, then launch dbRead
		// in a parallel go routine to send deliveries from db
		if inDb == 0 && quit != nil {
			logger.Debugf("syncDb[%v/%v] start dbRead")
	...
	logger.Infof("syncDb[%v:%v]: saved %v keys (saved counter at %v)", self.key.Log(), self.priority, inBatch, counter)
	close(self.done)
}
...
func (self *syncDb) writeSyncBatch(batch *leveldb.Batch) *leveldb.Batch {
	...
	if err != nil {
		logger.Warnf("syncDb[%v/%v] saving batch to db failed: %v", self.key.Log(), self.priority, err)
		return batch
	}
...
func (self *syncDb) dbRead(useBatches bool, counter uint64, fun func(interface{}, chan bool) bool) {
	...
	for {
		...
		logger.Debugf("syncDb[%v/%v]: new iterator: %x (batch %v, count %v)", self.key.Log(), self.priority, key, batches, cnt)
		...
		for n = 0; !useBatches || n < cnt; it.Next() {
			...
			// logger.Debugf("syncDb[%v/%v] - %v, batches: %v, total: %v, session total from db: %v/%v", self.key.Log(), self.priority, self.key.Log(), batches, total, self.dbTotal, self.total)
			more = fun(entry, self.quit)
			if !more {
				// quit received when waiting to deliver entry, the entry will not be deleted
				logger.Debugf("syncDb[%v/%v] batch %v quit after %v/%v items", self.key.Log(), self.priority, batches, n, cnt)
				break
			}
		...
		logger.Debugf("syncDb[%v/%v] - db session closed, batches: %v, total: %v, session total from db: %v/%v", self.key.Log(), self.priority, batches, total, self.dbTotal, self.total)
