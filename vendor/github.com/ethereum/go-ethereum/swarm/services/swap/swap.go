...
func NewSwap(local *SwapParams, remote *SwapProfile, backend chequebook.Backend, proto swap.Protocol) (self *swap.Swap, err error) {
	...
	if !ok {
		logger.Infof("invalid contract %v for peer %v: %v)", remote.Contract.Hex()[:8], proto, err)
	} else {
		...
		if err != nil {
			logger.Warnf("unable to set up inbox for chequebook contract %v for peer %v: %v)", remote.Contract.Hex()[:8], proto, err)
		}
	}
	...
	if !ok {
		logger.Warnf("unable to set up outbox for peer %v:  chequebook contract (owner: %v): %v)", proto, local.owner.Hex(), err)
	} else {
	...
	logger.Warnf("SWAP arrangement with <%v>: %v; %v)", proto, buy, sell)
...
func (self *SwapParams) deployChequebook(ctx context.Context, backend chequebook.Backend, path string) error {
	...
	logger.Infof("Deploying new chequebook (owner: %v)", opts.From.Hex())
	contract, err := deployChequebookLoop(opts, backend)
	if err != nil {
		logger.Errorf("unable to deploy new chequebook: %v", err)
		return err
	}
	logger.Infof("new chequebook deployed at %v (owner: %v)", contract.Hex(), opts.From.Hex())
	...
	self.lock.Unlock()
	if err != nil {
		logger.Warnf("error initialising cheque book (owner: %v): %v", opts.From.Hex(), err)
	}
	return err
}
...
func deployChequebookLoop(opts *bind.TransactOpts, backend chequebook.Backend) (addr common.Address, err error) {
	...
	for try := 0; try < chequebookDeployRetries; try++ {
		...
		if _, tx, _, err = contract.DeployChequebook(opts, backend); err != nil {
			logger.Warnf("can't send chequebook deploy tx (try %d): %v", try, err)
			continue
		}
		if addr, err = bind.WaitDeployed(opts.Context, backend, tx); err != nil {
			logger.Warnf("chequebook deploy error (try %d): %v", try, err)
			continue
		}
		return addr, nil
	}
...
func (self *SwapParams) newChequebookFromContract(path string, backend chequebook.Backend) error {
	...
	if err != nil {
		...
		if err != nil {
			logger.Warnf("unable to initialise chequebook (owner: %v): %v", self.owner.Hex(), err)
	...
	logger.Infof("auto deposit ON for %v -> %v: interval = %v, threshold = %v, buffer = %v)", crypto.PubkeyToAddress(*(self.publicKey)).Hex()[:8], self.Contract.Hex()[:8], self.AutoDepositInterval, self.AutoDepositThreshold, self.AutoDepositBuffer)
