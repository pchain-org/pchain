...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config, swapEnabled, syncEnabled bool, cors string) (self *Swarm, err error) {
	...
	logger.Debugf("Setting up Swarm service components")
	...
	// setup local store
	logger.Debugf("Set up local storage")

	self.dbAccess = network.NewDbAccess(self.lstore)
	logger.Debugf("Set up local db access (iterator/counter)")
	...
	logger.Debugf("Set up swarm network with Kademlia hive")

	// setup cloud storage backend
	cloud := network.NewForwarder(self.hive)
	logger.Debugf("-> set swarm forwarder as cloud storage backend")
	// setup cloud storage internal access layer

	self.storage = storage.NewNetStore(hash, self.lstore, cloud, config.StoreParams)
	logger.Debugf("-> swarm net store shared access layer to Swarm Chunk Store")
	...
	logger.Debugf("-> REmote Access to CHunks")

	// set up DPA, the cloud storage local access layer
	dpaChunkStore := storage.NewDpaChunkStore(self.lstore, self.storage)
	logger.Debugf("-> Local Access to Swarm")
	// Swarm Hash Merklised Chunking for Arbitrary-length Document/File storage
	self.dpa = storage.NewDPA(dpaChunkStore, self.config.ChunkerParams)
	logger.Debugf("-> Content Store API")
	...
	logger.Debugf("-> Swarm Domain Name Registrar @ address %v", config.EnsRoot.Hex())

	self.api = api.NewApi(self.dpa, self.dns)
	// Manifests for Smart Hosting
	logger.Debugf("-> Web3 virtual server API")
...
func (self *Swarm) Start(net *p2p.Server) error {
	...
	if self.swapEnabled {
		...
		logger.Debugf("-> cheque book for SWAP: %v", self.config.Swap.Chequebook())
	} else {
		logger.Debugf("SWAP disabled: no cheque book set")
	}

	logger.Warnf("Starting Swarm service")
	self.hive.Start(
		discover.PubkeyID(&net.PrivateKey.PublicKey),
		func() string { return net.ListenAddr },
		connectPeer,
	)
	logger.Infof("Swarm network started on bzz address: %v", self.hive.Addr())

	self.dpa.Start()
	logger.Debugf("Swarm DPA started")
	...
	logger.Debugf("Swarm http proxy started on port: %v", self.config.Port)

	if self.corsString != "" {
		logger.Debugf("Swarm http proxy started with corsdomain:", self.corsString)
	}
...
func (self *Swarm) SetChequebook(ctx context.Context) error {
	...
	logger.Infof("new chequebook set (%v): saving config file, resetting all connections in the hive", self.config.Swap.Contract.Hex())
	self.config.Save()
	self.hive.DropAll()
	return nil
}
