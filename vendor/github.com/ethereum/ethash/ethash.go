...
func (cache *cache) generate() {
	cache.gen.Do(func() {
		...
		logger.Debugf("Generating cache for epoch %d (%x)", cache.epoch, seedHash)
		...
		logger.Debugf("Done generating cache for epoch %d, it took %v", cache.epoch, time.Since(started))
	})
}
...
func (l *Light) Verify(block pow.Block) bool {
	...
	if blockNum >= epochLength*2048 {
		logger.Debugf("block number %d too high, limit is %d", epochLength*2048)
		return false
	}
	...
	*/
	if difficulty.Cmp(common.Big0) == 0 {
		logger.Debugf("invalid block difficulty")
		return false
	}
...
func (l *Light) getCache(blockNum uint64) *cache {
	...
	if c == nil {
		...
		if len(l.caches) >= l.NumCaches {
			...
			logger.Debugf("Evicting DAG for epoch %d in favour of epoch %d", evict.epoch, epoch)
		...
		if l.future != nil && l.future.epoch == epoch {
			logger.Debugf("Using pre-generated DAG for epoch %d", epoch)
			c, l.future = l.future, nil
		} else {
			logger.Debugf("No pre-generated DAG available, creating new for epoch %d", epoch)
		...
		if l.future == nil || l.future.epoch <= epoch {
			logger.Debugf("Pre-generating DAG for epoch %d", epoch+1)
...
func (d *dag) generate() {
	d.gen.Do(func() {
		...
		logger.Infof("Generating DAG for epoch %d (size %d) (%x)", d.epoch, dagSize, seedHash)
		...
		logger.Infof("Done generating DAG for epoch %d, it took %v", d.epoch, time.Since(started))
	})
}
...
func ethashGoCallback(percent C.unsigned) C.int {
	logger.Infof("Generating DAG: %d%%", percent)
	return 0
}
