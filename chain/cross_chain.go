...
func (cch *CrossChainHelper) VerifyTdmBlock(from common.Address, block []byte) error {
	...
	//1, data hash & num(tx)
	if !bytes.Equal(tdmBlock.Data.Hash().Bytes(), tdmBlock.DataHash.Bytes()) {
	...
	//2, last commit
	if !bytes.Equal(tdmBlock.LastCommitHash.Bytes(), tdmBlock.LastCommit.Hash().Bytes()) {
	...
	if !bytes.Equal(epoch.Validators.Hash(), tdmBlock.ValidatorsHash.Bytes()) {
